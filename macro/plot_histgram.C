...
void plot_histgram(std::vector<std::string> mhmass) {
	...
	auto leg = new TLegend(0.3,0.7,0.6,0.9);
//	gPad->SetLogy();

	string title_name="muon_pair_zmass";
	leg->SetHeader(title_name.c_str(),"C"); 
    ...
    for(int i=0;i<filename.size();i++){
    	...
    	hvariable[i] = new TH1F(hname.c_str(),title_name.c_str(),120,0,120);
